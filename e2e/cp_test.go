...
func TestCopyMultipleFilesMiddleWildcardedDirectoryToS3Bucket(t *testing.T) {
	t.Parallel()
	t.Run("CopyMultipleFilesMiddleWildcardedDirectoryToS3Bucket", func(t *testing.T) {
		for _, tc := range testCases {
			tc := tc
			t.Run(tc.storage, func(t *testing.T) {
				t.Parallel()
				...
				srcpath := filepath.ToSlash(workdir.Path()) + "/parent/c*1/name.txt"
				dstpath := fmt.Sprintf("%v://%v/newfolder/", tc.storage, bucket)
				...
				assertLines(t, result.Stdout(), map[int]compareFunc{
					0: suffix(`cp %v/parent/child1/name.txt %vnewfolder/parent/child1/name.txt`, workdir.Path(), dstpath),
					1: suffix(`cp %v/parent/child21/name.txt %vnewfolder/parent/child21/name.txt`, workdir.Path(), dstpath),
				})
...
func TestFlattenCopyMultipleFilesToS3Bucket(t *testing.T) {
	t.Parallel()
	t.Run("FlattenCopyMultipleFilesToBucket", func(t *testing.T) {
		for _, tc := range testCases {
			tc := tc
			t.Run(tc.storage, func(t *testing.T) {
				t.Parallel()
				...
				// assert objects
				assert.Assert(t, ensureObject(client, bucket, filepath.Base(subFile), subContent, tc.storage))
				assert.Assert(t, ensureObject(client, bucket, rootFile, rootContent, tc.storage))
...
func TestCopyMultipleFilesToS3WithPrefixWithoutSlash(t *testing.T) {
	t.Parallel()
	t.Run("CopyMultipleFilesToS3WithPrefixWithoutSlash", func(t *testing.T) {
		for _, tc := range testCases {
			tc := tc
			t.Run(tc.storage, func(t *testing.T) {
				t.Parallel()
				...
				assertLines(t, result.Stderr(), map[int]compareFunc{
					0: equals(`ERROR "cp <src> <dst>" requires <dst> to be a directory or end with "/"`),
				})

				// assert object does not exist
				assert.Assert(t, ensureObjectDoesNotExist(client, bucket, file1, tc.storage))
			})
		}
	})
}
...
func TestCopyMultipleFilesToS3WithPrefixWithSlash(t *testing.T) {
	t.Parallel()
	t.Run("CopyMultipleFilesToS3WithPrefixWithSlash", func(t *testing.T) {
		for _, tc := range testCases {
			tc := tc
			t.Run(tc.storage, func(t *testing.T) {
				t.Parallel()
				...
				assertLines(t, result.Stdout(), map[int]compareFunc{
					0: suffix(`cp %v %vprefix/%v`, filepath.ToSlash(workdir.Join(file1)), dstpath, file1),
					1: suffix(`cp %v %vprefix/%v`, filepath.ToSlash(workdir.Join(file2)), dstpath, file2),
				}, sortInput(true))

				// assert objects
				assert.Assert(t, ensureObject(client, bucket, "prefix/"+file1, content, tc.storage))
				assert.Assert(t, ensureObject(client, bucket, "prefix/"+file2, content, tc.storage))
...
func TestFlattenCopyMultipleFilesToS3WithPrefixWithSlash(t *testing.T) {
	t.Parallel()
	t.Run("FlattenCopyMultipleFilesToS3WithPrefixWithSlash", func(t *testing.T) {
		for _, tc := range testCases {
			tc := tc
			t.Run(tc.storage, func(t *testing.T) {
				t.Parallel()
				...
				assertLines(t, result.Stdout(), map[int]compareFunc{
					0: suffix(`cp %v %vprefix/%v`, filepath.ToSlash(workdir.Join(subFile)), dstpath, filepath.Base(subFile)),  
					1: suffix(`cp %v %vprefix/%v`, filepath.ToSlash(workdir.Join(rootFile)), dstpath, rootFile),
				}, sortInput(true))

				// assert objects
				assert.Assert(t, ensureObject(client, bucket, "prefix/"+filepath.Base(subFile), subContent, tc.storage))
...
func TestFlattenCopyLocalDirectoryToS3WithPrefixWithSlash(t *testing.T) {
	t.Parallel()
	t.Run("FlattenCopyLocalDirectoryToS3WithPrefixWithSlash", func(t *testing.T) {
		for _, tc := range testCases {
			tc := tc
			t.Run(tc.storage, func(t *testing.T) {
				t.Parallel()
				...
				assertLines(t, result.Stdout(), map[int]compareFunc{
					0: suffix(`cp %v%v %vprefix/%v`, srcpath, subFile, dstpath, filepath.Base(subFile)),
					1: suffix(`cp %v%v %vprefix/%v`, srcpath, rootFile, dstpath, rootFile),
				}, sortInput(true))
